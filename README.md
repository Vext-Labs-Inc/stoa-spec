# Janus v0.1 — Specification

**Status:** Draft 1 (open standard, public review pending)
**Author:** Vext Labs — Architecture thread
**Date:** 2026-05-01
**Scope:** Wire-level protocol that any SaaS vendor can implement to expose itself to autonomous agents as a first-class consumer. Open standard; consumed by Vext Pro Hive AND by every other agent platform that adopts it.
**Companion:** [../VEXT_PRO_HIVE_BACKEND.md](../VEXT_PRO_HIVE_BACKEND.md) §9 (the adapter layer that consumes Janus contracts), [BRAND.md](BRAND.md) (naming + GTM)
...
**Janus is the answer:** SaaS products ship two front doors — one for humans (the UI), one for machines (a typed, contract-first, self-describing surface designed for agents). When the UI redesigns, the Janus contract is unchanged; the agent never breaks.

Concretely, an Janus product publishes a **capability manifest** (JSON Schema + OpenAPI 3.1 + MCP server) listing every action an agent can take, with typed inputs, typed outputs, side-effect annotations, idempotency keys, and rollback semantics. Every response uses a **deterministic state envelope**. Every error is a **typed, remediable contract**. Permissions are **capability-scoped**, not endpoint-scoped.
...
Janus sits **above** OpenAPI 3.1, JSON Schema 2020-12, and MCP — it does not replace them. A vendor with a clean OpenAPI spec is 80% of the way there. Janus adds the missing 20% that agents actually need:
...
The minimum viable Janus implementation is an existing OpenAPI 3.1 spec + an `x-janus` extension block per operation + a `/.well-known/janus.json` discovery file.

---

## 2. Discovery

Every Janus-conformant product MUST serve a discovery document at:

```
GET https://<host>/.well-known/janus.json
```

```jsonc
{
  "spec_version": "janus-0.1",
  "vendor": {
    "name": "Acme CRM",
  ...
  "manifest_url": "https://acme-crm.com/api/.well-known/janus-manifest.json",
  ...
  "conformance": {
    ...
    "report_url": "https://acme-crm.com/conformance/janus-0.1-report.json"
  }
}
```

`spec_version` MUST be a string of form `janus-<major>.<minor>`. Agents discovering a vendor at a higher minor version they don't recognize SHOULD proceed assuming forward-compatibility (additive changes only); higher major version requires explicit support.
...
Vendors register custom domains by including them in the manifest's optional `domains` block. Reserved: `janus` (used for spec metadata) and `vext` (Vext-only).
...
Every Janus response MUST be wrapped in a state envelope. This is the part that kills "page 2 of 17" hallucinations.
...
Every Janus error response MUST follow:
...
Janus scopes are **capability-level**, not endpoint-level. The pattern:
...
The Hive's planner requests the smallest scope that covers the plan. If a step needs `crm.contacts.create` only, the planner asks for `crm:contacts:write`, not `crm:*:*`. The Janus layer audits actual capability use against scope grant per-call; over-scoped grants generate a warning in the conformance report.
...
| `api_key` | Simple service-to-service (header: `X-Janus-Key`) |
...
The Vext Janus marketplace surfaces vendors by level. `full`-conformant vendors get the prominent shelf; `core` vendors get listed but flagged.

---

## 9. Conformance test suite (outline)

The suite is automatable and runs against a candidate Janus host. Open-source under MIT.

### 9.1 What it tests

For each declared capability:

1. **Discovery roundtrip.** GET `/.well-known/janus.json` resolves; manifest URL resolves; manifest validates against the v0.1 JSON Schema.
...
`https://github.com/vext-labs/janus-conformance` — open-source, MIT.

---

## 10. The Janus marketplace

Out of scope for v0.1 spec; sketch only:

- A directory at `https://janus.tryvext.com/marketplace` listing every conformant vendor.
- Search by capability ID (`find me everything that does crm.contacts.create`).
- Vext takes a revenue share on agent-traffic billed through the vendor's Janus surface (think: Plaid for SaaS-to-agent traffic).
- Vendors who publish open Janus surfaces but don't bill via Vext are still listed; the revenue share applies only to vendors who use the Hive's billing rails.
- Vext-published adapters (the OpenAPI-to-Janus shims) for major SaaS vendors not yet conformant are listed as "Vext-attested" — same compliance level rules, but Vext maintains the shim.

---

## 11. Versioning

- Spec version is `janus-<major>.<minor>`. v0.1 is initial draft.
- v1.0 ships once the first 25 vendors hit `full` conformance.
- Spec changes are RFC-driven; live on GitHub at `https://github.com/vext-labs/janus-spec`.
...
## 12. Why an agent should prefer Janus

| Characteristic | Browser scraping | OpenAPI 3.1 | Janus |
|---|---|---|---|
...
Higher-fidelity adapters cost less to run (fewer retries, no scraping retries on layout drift, predictable rate limits) and produce better outcomes (no "the bot booked the wrong meeting because the calendar UI changed"). For Vext Pro Hive, Janus is the default ingest; browser scraping is the explicit fallback we report to the user as low-fidelity.
...
Spec is published under CC-BY-4.0. Conformance test suite is MIT. Reference adapter implementations under Apache-2.0.

The spec is governed by an open RFC process; Vext Labs is the editor of v0.1 but not the long-term steward — once 5+ independent vendors ship `full` conformance, the spec moves under a foundation (Linux Foundation or equivalent).
